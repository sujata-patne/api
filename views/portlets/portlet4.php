...
<?php if(sizeof($storeObj->getPortletWallpapers($CURRENT_PORTLETID)) > 0 ) {  ?>
    <div style="height: 30px; background: #ccc; padding-top: 6px;"><h5 style="margin-top: 7px;">Photos</h5></div>
<?php } ?>
...
<?php $i = 0;
	 foreach ($storeObj->getPortletWallpapers($CURRENT_PORTLETID) as $key => $value) {
        ...
        <td align="center">
            <a href="../<?=$SUBPARAM.$pricepoint?>&f=home&t=<?=$value->contentTypeMD5?>&n=<?=base64_encode($storeObj->getDifferentFileNames($value->cf_url,$PORTLET_CONTENT_TYPE,$PORTLET_RESOLUTION))?>&m=<?=$value->cf_cm_id?>&d=<?=$value->cd_id?>&i=<?=$value->cf_template_id?>">
    ...
    <tr >
        <td height="30" colspan="3" align="right">
            <?php if($USERSTATUS == 'NEWUSER' || $USERSTATUS == 'UNKNOWN' || $USERSTATUS == 'UNSUBSCRIBED' ){
                ...
                <a href="../<?=$SUBPARAM.$pricepoint?>&f=home&n=<?=base64_encode(1)?>" style="text-decoration:none;">More >></a>
            <?php }
                }else{
                    if(count($storeObj->getPortletWallpapers($CURRENT_PORTLETID)) > $THUMBNAIL_LIMIT) { ?>
