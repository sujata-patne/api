...
<div style="height: 30px; background: #ccc; padding-top: 6px;"><h5 style="margin-top: 7px;">Videos</h5></div>
<?php } ?>
...
<?php $i = 0;
	 foreach ($storeObj->getPortletVideos($CURRENT_PORTLETID) as $key => $value) {
        ...
        <td align="center">
            <a href="../<?=$SUBPARAM.$pricepoint?>&f=home&t=<?=$value->contentTypeMD5?>&n=<?=base64_encode(1)?>&m=<?=$value->cf_cm_id?>&d=<?=$value->cd_id?>">
            	<img src="<?=$THUMBURL?><?=$value->cf_cm_id?>_thumb_125_125.jpg" width="125" height="125" alt="" /></a>
            <br />
        </td>
<?php }else{    //If user is subscribed :  clicking on thumbnail for video will download the same in low res. ?>
         <td align="center">
